from vec import Vec
from mat import Mat
from bitutil import noise, bits2mat, mat2bits, str2bits, bits2str
from GF2 import one
...
R = listlist2mat([[one, one, 0, one, 0, 0, 0],
                  [one, one, 0, one, 0, one, one],
...
def find_error(e):
    ...
    h = mat2coldict(H)
    for k in h:
        if h[k] == e: return Vec(set(h.keys()), {k:one})
    return Vec(set(h.keys()), {})
