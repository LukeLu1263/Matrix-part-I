...
def lin_comb_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return sum(v[c] * mat2coldict(M)[c] for c in v.D)
#return Vec(M.D[0],{k:sum(v[c]*M[r,c] for r in M.D[0] for c in M.D[1]) for k in v.D})


## Problem 12
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return sum(v[r] * mat2rowdict(M)[r] for r in v.D)
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec(set(M.D[0]),{r: mat2rowdict(M)[r] * v for r in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec(set(M.D[1]), {c: v * mat2coldict(M)[c] for c in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    col_dict = {c: A*mat2coldict(B)[c] for c in B.D[1]}
    return Mat((A.D[0], B.D[1]), {(r,c): col_dict[c][r] for r in A.D[0] for c in col_dict})
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    row_dict = {r: mat2rowdict(A)[r]*B for r in A.D[0]}
    return Mat((A.D[0], B.D[1]), {(r,c): row_dict[r][c] for r in row_dict for c in B.D[1]})
...
def dot_prod_mat_mat_mult(A, B):
    ...
    row_A = mat2rowdict(A)
    col_B = mat2coldict(B)
    return Mat((A.D[0],B.D[1]), {(r,c): row_A[r] * col_B[c] for r in A.D[0] for c in B.D[1]})
