...
def add(A, B):
    ...
    assert A.D == B.D
    for k in A.f:
        if k not in B.f: B.f[k] = 0
    for k in B.f:
        if k not in A.f: A.f[k] = 0
    return Mat(A.D,{k: A.f[k]+B.f[k] for k in A.f})
...
def scalar_mul(M, alpha):
    ...
    if alpha == 0:
        ...
        return Mat(M.D, {(r,c): alpha*v for (r,c),v in M.f.items()})
